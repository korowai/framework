...
algorithms, possibly using PCRE and patterns provided by the
...
expressions. The syntax of :ref:`lib.rfc.rfc2849.changerecord` is
refactored to an equivalent form, which introduces custom
:ref:`lib.rfc.rfc2849.changerecord-init` rule.
...
.. list-table:: VERSION_SPEC: capture groups
   ...
   * - ``version_number``
     - The whole substring between ``"version:"`` and nearest
       :ref:`lib.rfc.rfc2849.eol`, if the whole substring matches
       :ref:`version-number <lib.rfc.rfc2849.version-number>` rule.
   * - ``version_error``
     - The erroneous part of the substring between ``"version:"`` and nearest
       :ref:`lib.rfc.rfc2849.eol` (the part that failed to match
       :ref:`lib.rfc.rfc2849.version-number` rule), if the whole substring
       failed to match the :ref:`lib.rfc.rfc2849.version-number`.
...
and is implemented in :class:`Korowai\\Lib\\Rfc\\Rfc2849` class as
