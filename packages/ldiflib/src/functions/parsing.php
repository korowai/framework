<?php
/**
 * @file src/functions/parsing.php
 *
...
use Korowai\Lib\Ldif\ParserStateInterface as StateInterface;
 ...
 */
function parseBase64Decode(StateInterface $state, string $string, ?int $offset = null) : ?string
{
 ...
 * @return string|null Returns the decoded data or null on error.
 */
function parseUtf8Check(StateInterface $state, string $string, ?int $offset = null) : bool
{
 ...
 */
function parseMatchRfcRule(StateInterface $state, RuleInterface $rule, array &$matches = null) : bool
{
    $cursor = $state->getCursor();

    $matches = matchAhead('/\G'.$rule.'/D', $cursor, PREG_UNMATCHED_AS_NULL);
    if (empty($matches)) {
        if (!$rule->isOptional()) {
 ...
 *
 *      bool completion(StateInterface $state, array $matches, &$value = null);
 *
 ...
 */
function parseWithRfcRule(StateInterface $state, RuleInterface $rule, callable $completion, &$value = null) : bool
{
    if (!parseMatchRfcRule($state, $rule, $matches)) {
        $value = null;
        return false;
    }
